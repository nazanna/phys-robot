...
from constants import users_db_name, responses_db_name, UPLOAD_FREQUENCY, workdir, MAX_IMAGES_PER_QUESTION
from upload_to_google_sheets import upload_student_answers_to_sheets
from questions import QUESTIONS, QUESTIONS_FOR_GRADE
import logging
...
async def get_users_grade(user_id: int, context: ContextTypes.DEFAULT_TYPE):
    if 'grade' in context.user_data:
    ...
    if len(rows) == 0:
        ...
        context.user_data['grade'] = 7 # TODO: default grade, must be impossible
    else:
        context.user_data['grade'] = rows[0][0]
    return context.user_data['grade']
...
async def send_question(message, user_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    if question_index < len(QUESTIONS_FOR_GRADE[grade]):
        keyboard = [
        ...
        question_number = QUESTIONS_FOR_GRADE[grade][question_index] - 1
        
        current_images = await get_images_for_question(question_number)

        if len(current_images) == 1:
            await message.reply_photo(photo=open(current_images[0], 'rb'), caption=QUESTIONS[question_number], reply_markup=reply_markup)
        else:
            media = []
